...
public class UiLanguage: MonoBehaviour {
	...
	void setKor() {
		...
		kor.Add("Tutorial_32", "잡몸을 대상으로 연습해 보자. 방사능 괴물에 몸이 닿으면 어떻게 될지는 상식적으로 생각하자.");
		...
		kor.Add("Tutorial_34", "이제 내려치기 검법으로 약한 적들을 물리치면서 모험을 시작해 보자! 나머지 검법들은 차차 진행하면서 알려주겠다.");

		kor.Add("Tutorial_41", "적에게 피해를 입으면 짧은 시간 동안 몸이 빨갛게 되면서 이동속도가 느려지고 공격불가 생태가 된다. 허접해 보이는 게임이라고 쉽게 클리어할 수 있다고 생각하면 오산이다.");

		kor.Add("Tutorial_42", "당신을 쫒아오는 적들에게 내려치기 검법을 시전하다가는 쳐맞기 좋을 것이다. 이제 두번째 검법 휘둘러베기를 전수하겠다. W를 누르면 눌러서 360도 공격을 이용해 뒤에서 쫓아오는 적을 제압하자.");
		kor.Add("Tutorial_43", "휘둘러베기 검법은 사방을 한번에 공격할 수 있고, 이동하면서 시전할 수 있다는 장점이 있다 (시전 후 짦은 시간 동안은 이동할 수 없다). 한편 스태미너 소모량이 크고 내려치기보다 데미지가 약하다는 것이 단점이다.");
